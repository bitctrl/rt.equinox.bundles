		...
		try {
			AccessController.doPrivileged(new PrivilegedExceptionAction<Boolean>() {
				...
				public Boolean run() throws Exception {
					...
					// Hack for unknown MIME type.
					if (contentType == null) {
						if (filename.endsWith(".mp4")) { //$NON-NLS-1$
							contentType = "video/mp4"; //$NON-NLS-1$
						}
					}
					...
					// Hack for Internet Explorer and Edge. This browsers do not send range
					// requests for videos, but handle response with range. Without this hack, RAP
					// will freeze in Internet Explorer and Edge.
					if (range == null && contentType != null && contentType.startsWith("video")) { //$NON-NLS-1$
						range = new Range();
